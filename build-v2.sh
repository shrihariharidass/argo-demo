...
echo "Building and pushing v2 for AMD64 (EKS compatible)..."
...
# Build and push v2 for AMD64 platform (EKS compatible)
docker build --platform linux/amd64 -t ${ECR_REPO_URI}:v2 .
docker push ${ECR_REPO_URI}:v2

# Restore original
mv index-v1-backup.html index.html

echo "v2 (AMD64) pushed successfully: ${ECR_REPO_URI}:v2"